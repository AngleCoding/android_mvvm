...
class SlidingTabLayout @JvmOverloads constructor(
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        ...
        mIndicatorHeight = ta.getDimension(
            ...
            dp2px((if (mIndicatorStyle == STYLE_TRIANGLE) 4 else (if (mIndicatorStyle == STYLE_BLOCK) -1 else 2f)).toFloat()).toFloat()
        )
        mIndicatorWidth = ta.getDimension(
            R.styleable.SlidingTabLayout_tl_indicator_width,
            dp2px((if (mIndicatorStyle == STYLE_TRIANGLE) 10 else (-1).toFloat()).toFloat()).toFloat()
        )
        mIndicatorCornerRadius = ta.getDimension(
            R.styleable.SlidingTabLayout_tl_indicator_corner_radius,
            dp2px((if (mIndicatorStyle == STYLE_BLOCK) -1 else 0f).toFloat()).toFloat()
        )
        ...
        indicatorMarginTop = ta.getDimension(
            ...
            dp2px((if (mIndicatorStyle == STYLE_BLOCK) 7 else 0f).toFloat()).toFloat()
        )
        ...
        mTabPadding = ta.getDimension(
            ...
            (if (mTabSpaceEqual || mTabWidth > 0) dp2px(0f) else dp2px(20f).toFloat()).toFloat()
        )
        ta.recycle()
    }
    ...
    fun setViewPager(vp: ViewPager?, Titles: ArrayList<String>?) {
        check(!(vp == null || vp.adapter == null)) { "ViewPager or ViewPager adapter can not be NULL !" }
        mViewPager = vp
        mTitles = ArrayList()
        mTitles = Titles
    ...
    fun addNewTab(title: String) {
        ...
        val pageTitle = if (mTitles == null) mViewPager!!.adapter!!.getPageTitle(
            tabCount
        ) else mTitles!![tabCount]
        ...
        for (i in 0 until tabCount) {
            ...
            if (tab_title != null) {
                ...
                tab_title.setTextSize(
                    ...
                    if (isSelect) mTextsize + 12f else mTextsize
                )
                //选中后文字变粗
    ...
    init {
        ...
        if (height == ViewGroup.LayoutParams.MATCH_PARENT.toString() + "") {
            ...
            @SuppressLint("ResourceType") val a = context.obtainStyledAttributes(attrs, systemAttrs)
